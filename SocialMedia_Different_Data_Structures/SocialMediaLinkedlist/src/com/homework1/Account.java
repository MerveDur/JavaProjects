...
public class Account {
     ...
     * The Array of posts
     */
    private LinkedList<Post> posts = new LinkedList<Post>(); // works fine
    /**
     ...
     */
    private LinkedList<Account> blockedAccounts = new LinkedList<Account>();
    /**
    ...
    */
    public Account(LinkedList<Account> accounts,int accontSize,int accountid, String username, String birthdate, String location, boolean loginStatus)
    {
     ...
     */
    public void setFollowing(LinkedList<Account> following,int newFollowingSize)
    {
     ...
     */
    public void setFollowers(LinkedList<Account> followers, int newFollowerSize)
    {
     ...
     */
    public  void viewingInbox(LinkedList<Account> senderList,int senderListSize)
    {
        if(this.loginStatus==true)//Checking that the account is logged in
        {
     ...
     * @param senderListSize  An int containing a sender list size
     */
    public  void viewingOutbox(LinkedList<Account> senderList,int senderListSize)
    {
        if(this.loginStatus==true)//Checking that the account is logged in
        {
     ...
     */
    public void  viewingPostsInteractions(Account tempAccount,LinkedList<Account> senderList,int senderListSize)
    {
        int commentOrder=1;
                ...
                {//Followers who like the post are displayed
                    ...
                    for (int j = 0; j < tempAccount.getPosts().get(i).getLikesSize(); j++)
                    {
                        try
                        {
                            ...
                            {
                                if(senderList.get(k).getAccountid()== tempAccount.getPosts().get(i).getLikes().get(j).getAccountId())
                                {
                ...
                if (tempAccount.getPosts().get(i).getCommentsSize()>0)
                {
                    ...
                    {// Comments on the post are displayed
                        ...
                        try
                        {
                            for (int k = 0; k < senderListSize; k++)
                            {
                                if(senderList.get(k).getAccountid()== tempAccount.getPosts().get(i).getComments().get(j).getAccountId())
                                {
    ...
    {// Increase the size of the block array and add the new block account
        ...
        LinkedList<Account> newfollowers = new LinkedList<Account>();
        LinkedList<Account> newfollowing = new LinkedList<Account>();
        int j=0;
        try
        {
            if (this.isLoginStatus() == true)//Checking that the account is logged in
