...
public class SelectionSort {
    ...
    /**
     * Sorts the original map using selection sort algorithm
     */
    public void sort()
    {
        int min=0;
        for (int i = 0; i < aux.size()-1; i++)
        {
            ...
            {
                if (originalMap.map.get(aux.get(j)).count < originalMap.map.get(aux.get(min)).count)
                {
                    min = j;
                }
            }

            String swapTemp = aux.get(min);
            aux.set(min, aux.get(i));
            aux.set(i, swapTemp);
        }
